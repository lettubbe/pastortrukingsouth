...
const EditSection = () => {
  ...
  const isExpandPhase = scrollProgress > 0 && scrollProgress < 0.9
  const isFullScreenPhase = scrollProgress >= 0.9

  // Calculate text positions (move inward as scroll progresses)
  const textMoveProgress = isExpandPhase
    ? Math.min(1, scrollProgress / 0.9) // 0 to 1 over 0-0.9 range
    : isFullScreenPhase
      ? 1
      : 0
  ...
  if (isExpandPhase || isFullScreenPhase) {
    const expandProgress = Math.min(1, scrollProgress / 0.9) // 0-1 over 0-0.9
    ...
    >
      ...
      <div
        style={{
          ...
          alignItems: 'center',
          position: isExpandPhase || isFullScreenPhase ? 'fixed' : 'relative',
          top: 0,
          left: 0,
          right: 0,
          zIndex: 1,
      ...
      <div style={{
        position: isExpandPhase || isFullScreenPhase ? 'fixed' : 'absolute',
        top: 0,
        left: 0,
        right: 0,
        bottom: isExpandPhase || isFullScreenPhase ? 0 : 'auto',
        height: isExpandPhase || isFullScreenPhase ? '100vh' : '100%',
